...
- Workstream registry (adopted 2026-08-16): workstream branches never edit `tasks/todo.md`. The coordinating session holds that pen and opens and closes registry entries as small direct-to-main commits (`ALLOW_MAIN_COMMIT=1`, checks green). Rationale: when every parallel branch edited the same Active-workstreams bullets, the second and third merges conflicted there every time.
