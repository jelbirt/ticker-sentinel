...
**Share-count basis (2026-08-15):** a stock split rebases every share count the source serves, retroactively and only for the ~5 quarters it still returns (CRWD 4:1 on 2026-07-02, NOW 5:1 on 2025-12-18), while older quarters survive in the cache on the pre-split basis. On merge, cached share rows are rebased by the factor the overlapping quarters agree on, so the row never mixes bases. On top of that, every read scrubs `diluted_shares` cells that fall outside 0.33x to 3x of the provider's current shares outstanding (a units-for-thousands slip hit NOW on 2026-08-07) or that sit behind a step of more than 1.5x from the next newer quarter; scrubbed cells become n/a with a data note, so `dilution` and the repriced market cap degrade instead of quoting a number from the wrong basis.
---
