...
**History depth and R40 trend warm-up (2026-08-15):** yfinance returns roughly 5 quarters at a time and the cache accumulates one more each quarter, so the committed cache holds 6 to 7 today. Growth needs 8 quarters and `r40_trend` needs 12 (TTM now against TTM at -4Q, each with a year of denominator behind it), which leaves the F-score trend term, R40-inflection change detection, the Deterioration watch R40 signal and the weekly digest decay gate inert until roughly mid-2027 on organic accumulation alone. Two responses ship together. (1) Disclosure: the report carries one aggregate data note while the metric is warming up ("R40 trend warming up: n/a for N of M scored names"), which self-erases once every scored name has a trend. (2) Mechanism: `python -m sentinel.backfill` is a one-time tool that deepens the cache from the SEC EDGAR companyfacts XBRL API (free, no key, declared User-Agent, throttled) up to the 16-quarter cap. EDGAR is never wired into the scheduled run; steady state stays yfinance. Income-statement facts come per quarter from 10-Qs, cash-flow facts are differenced out of the year-to-date figures, and Q4 is derived from the fiscal year minus the first three quarters. Nothing is written until a ticker passes the verification gate: every quarter present in both the EDGAR-derived frame and the cached yfinance frame must agree on every mapped field within 1 percent, or within 100,000 absolute for near-zero values. One mismatch rejects that ticker whole (no partial trust), and a ticker with no overlap to check is rejected as unverified rather than trusted. Accepted tickers keep every cached yfinance value: the merge appends quarters older than the cache and fills the EMPTY cells inside it (yfinance leaves hollow shell columns that block every TTM window spanning them), but never overwrites a cached value. Amendment 1 (approved 2026-08-15) also narrowed the backfilled fields to what the history actually feeds (`revenue`, `ocf`, `capex`, `sbc`; `operating_income` and `d_and_a` are read only from the newest quarter, so they stay NaN and stay out of the gate), and made capex a composite of its base tag plus the software-capitalization tags, because yfinance's "Capital Expenditure" bundles capitalized software development that a single PP&E tag misses. MNDY is skipped a priori: as a 20-F/6-K filer its XBRL facts are annual and half-year only, with no 3-month periods to derive quarters from (wording corrected 2026-08-16 after a read-only EDGAR check on CIK 1845338: MNDY does file US-GAAP XBRL and it reconciles with our cache to the dollar, so the blocker is period granularity, not missing or disagreeing data, and the skip stands). Bench names have no cache to verify against, so the tool fetches their current yfinance statements to create the overlap, and cache pruning now keeps the bench (`cfg.cache_tickers`) so the seeded parquets survive the next scheduled run. `--dry-run` is the default and writes nothing; `--apply` is a one-time owner-gated run, because `data/cache/` is the scheduled bot's pen. Spec and decisions: `tasks/spec-history-backfill.md`. Amendment 2 (approved 2026-08-15) dropped `diluted_shares` from the backfill too: dilution reads only the newest five yfinance-served quarters, and EDGAR as-filed counts sit on the filing-date basis, which for split tickers would fight the share-count basis guard below; it also added `PaymentsForSoftware` as a fifth capex addend (RBRK, OKTA and DT file it and yfinance folds it into their capex).
**Share-count basis (2026-08-15):** a stock split rebases every share count the source serves, retroactively and only for the ~5 quarters it still returns (CRWD 4:1 on 2026-07-02, NOW 5:1 on 2025-12-18), while older quarters survive in the cache on the pre-split basis. On merge, cached share rows are rebased by the factor the overlapping quarters agree on, so the row never mixes bases. On top of that, every read scrubs `diluted_shares`: cells outside 0.33x to 3x of the provider's current shares outstanding are dropped (a units-for-thousands slip hit NOW on 2026-08-07), and a step of more than 1.5x between neighbouring quarters splits the row into two bases, keeping the side whose readings sit closer to shares outstanding (recency alone cannot arbitrate: the corrupt cell can be the newest one); with no reference available, every reading drops rather than guessing. Scrubbed cells become n/a with a data note, and the scrub is read-time only: the committed cache always keeps the unsanitized merge, so a false positive can never permanently destroy accumulated history.
---
...
1. **Header:** date, build time, run type (scheduled/ad hoc), data freshness notes. **Display timezone (2026-08-17):** `report.timezone` (default America/New_York) was parsed but unread; it now renders the header's build time as a zone-labelled wall clock ("built 06:02 EDT"), so a report written by a UTC runner reads in the owner's clock. Presentation only, and deliberately so: the run date beside it is the run's own `date.today()`, the same value that names `reports/YYYY-MM-DD`, keys change detection and lands in `run_history.json`, so changing the config can never move which day a run belongs to (pinned by tests). An unloadable or misspelled zone degrades to a UTC stamp plus a data note naming the bad value, never a crash.
2. **Market context strip:** SPY 1d/1m change, watchlist median R40.
3. **What changed today (Phase 4, 2026-08-06):** every move since the prior run that cleared a config threshold (`changes:` block): composite/rank moves, flag transitions, R40 trend inflections, trend-state changes, new crosses, estimate-revision swings, short-interest readings, universe changes. Quality-signal arrows (up improving, down worsening). A quiet day renders exactly one line; the first run ever renders a "no prior state" line. Baselines come from the committed `data/cache/run_history.json` (25-run retention, written by real full-universe runs only, committed by the scheduled bot alongside the parquet cache; ticker-subset runs skip detection with a note; dry runs read fixture state and never write).
...
9. **Bench (unranked) (2026-08-16):** the reserve swap candidates (`bench:` in watchlist.yaml), shadow-scored every run on exactly the universe basis (same F, T, composite, flags) so a rotation decision compares like with like, and rendered with the strong-table columns plus a one-line note that they are comparison references, not recommendations. Quarantine is structural: bench cards live in their own list from fetch to render, so they never reach breadth ranking, the strong/weak tables, movers and alerts, the technicals-only table, the deterioration watch, the watchlist median, change detection or the CSV/JSON artifacts. Between-quarter signals (estimates, short interest, insiders) are deliberately not fetched for bench names, since they are informational and never score inputs; those cells read n/a and the API load stays flat (cached fundamentals plus 4 extra symbols in the one batched price call). Snapshots persist under a sibling `bench` key in `run_history.json` (additive; the schema version does not move) so the weekly digest can show week-window bench moves on the attention list's basis.
10. **Appendix/footer:** methodology one-liner, disclaimer, link to repo run.
...
- **Phase 4.2 (rotation evidence, 2026-08-16):** the round-1 refresh showed the digest could not tell a 2-run coverage gap from a 1-run blip, pooled fetch problems with business decay in one flags cell, and gave the bench no numbers at all. Answers, recorded in SPEC.md 7.0.1: coverage gaps carry a consecutive-runs-missing streak; attention entries carry a consecutive decay-gate-hit streak beside the hit count (the gate itself is unchanged); the attention table splits data-quality flags from business flags; the bench is shadow-scored every run (section 7 item 9) and the digest reports its week-window composite moves on the attention list's basis; and `--json PATH` writes a machine-readable twin, uploaded as a weekly-refresh artifact, as the substrate for the refresh #3 automate-vs-manual call. `changes.retention_runs` 12 -> 25 so history spans five digest windows.
- **Phase 4.3 (level rule in the digest, 2026-09-20):** rounds 5 and 6 adopted a level rule (SPEC.md 7.0.1): a name in the bottom `changes.weak_bottom_n` by fundamental score on the last run of `changes.weak_windows` consecutive digest windows, while `r40_trend`-live with no data-quality flag at each, surfaces as "persistently weak", a separate row kind from decay that asks for a bench comparison on the fundamental leg and never proposes a swap by itself. Round 6 read it off the level table by hand and made the first swap (S to the bench, SHOP promoted); from refresh #7 the digest computes it (a table under the attention list with the fundamental score, `r40_fcf` pass or fail against the shared `R40_BAR`, rank, status and business flags), and the bench table carries the same fundamental and `r40_fcf` columns so the comparison is two columns of one issue. Still read-only over history.
...
| Run-history state corrupted or growing | versioned JSON with wrong-shape detection degrading to "change detection reset" note (never a crash); 25-run retention pruned on every write (raised from 12 on 2026-08-16 so the file spans five weekly digest windows; roughly 250-350 KB); sorted-key pretty printing keeps bot-commit diffs reviewable. The `bench` block added the same day is a sibling key, so old readers ignore it and the schema version stays 1: bumping would reset change detection on every deployed reader for a purely additive change |
